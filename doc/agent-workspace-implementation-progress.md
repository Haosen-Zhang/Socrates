...
## Product-polish checkpoint — 2026-07-17

- Added forward-only migration 008 for app-local project archival, room-to-workspace binding, and session archival. Project removal unregisters only Socrates metadata: it unbinds conversations and never deletes the selected directory. Active sessions and workspace-scoped MCP configuration block project removal.
- The chat sidebar now groups legacy rooms and Agent sessions under each project, including a no-project group. Projects, rooms, and sessions expose rename/archive/remove actions; archived entries remain recoverable in the archive dock.
- Restored message timestamp/copy/rewind actions for Room, Single Agent, and Multi-Agent timelines. Rewind is explicitly context-only: it removes later persisted conversation state and does not claim to reverse file or shell side effects.
- Restored the legacy multi-member Room flow for Chat: Chat rooms may select multiple agents and then use the existing discussion/debate/order/round configuration composer. The structured Multi-Agent Session path remains available as the advanced plan-and-execution workflow.
- Composer responsiveness now uses optimistic local user turns, immediate draft clearing, animation-frame batched Room deltas, and non-queued streaming follow-scroll. This does not change provider, tool, workspace, approval, or execution permissions.
- Verification: `bun test` (219 pass / 731 assertions), `bun run typecheck`, `bun run lint`, `bun run --cwd apps/desktop build`, and `bun run --cwd apps/desktop tauri build --bundles app` all pass. `tauri dev` could not acquire port 1420 because an existing local Vite process already owned it; no process was terminated.
