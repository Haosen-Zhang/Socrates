const SENSITIVE_ASSIGNMENT = /\b(api[_-]?key|authorization|token|secret|password|proxy[_-]?password)\b\s*[:=]\s*([^\s,;]+)/giu;
...
export function redactObject(value: unknown, exactSecrets: readonly string[] = []): unknown {
  ...
  return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([key, item]) => [
    key, /api[_-]?key|authorization|token|secret|password/iu.test(key) ? "[REDACTED]" : redactObject(item, exactSecrets),
  ]));
}
